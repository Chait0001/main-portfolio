      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, index) => {
            const direction = index % 3 === 0 ? -120 : index % 3 === 2 ? 120 : 0;
            const yDir = index % 3 === 1 ? 60 : 0;

            return (
              <motion.div
                key={project.title}
                initial={{ opacity: 0, x: direction, y: yDir }}
                ...
                whileHover={{ 
                  y: -8,
                  boxShadow: '0 20px 40px rgba(0, 200, 160, 0.15)',
                  borderColor: 'rgba(0, 200, 160, 0.4)'
                }}
                className="bg-white/[0.03] rounded-none p-6 border border-white/[0.06] transition-all duration-300 group relative z-10"
              >
